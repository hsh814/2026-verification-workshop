...
From map.bst Require Export MapHeader.
...
Section resources.
  ...
  (** TODO 1(a): exclusive client ownership of one handle's model. *)
  Definition is_map (handle : loc) (model : amap) : iProp Σ.
  Admitted.
End resources.
...
Module MapA. Section MapA.
  ...
  (** TODO 1(b): [new_map] creates an empty mathematical map. *)
  Definition new_map_spec : fspec.
  Admitted.

  (** TODO 1(c): [insert] updates one key with [<[key := value]> model]. *)
  Definition insert_spec : fspec.
  Admitted.

  (** TODO 1(d): [delete] removes one key with [delete key model]. *)
  Definition delete_spec : fspec.
  Admitted.

  (** TODO 1(e): [get] returns [model !! key] without changing [model]. *)
...
(** TODO 1(f): allocate the initially empty authoritative registry. *)
Lemma map_alloc
